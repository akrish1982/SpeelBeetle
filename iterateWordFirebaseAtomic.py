...
with open('allWordList-a-h2.csv', encoding='utf-8') as csv_file:
    ...
    for index, row in df_wordList.iterrows():
        ...
        try:
            ...
            etymology = ""
        print(etymology)
        #pron.append(etymology)
        
        dct = {'Words':word,'Links':webURL,'word_form':wordForm,
                    'pronunciation':soundStr,'etymology':etymology,'definition':definition,
                    'example':example}
        data = firebase_admin.db.reference(url='https://spell-beetle.firebaseio.com')
        wordData = data.child('words').push(value=dct)    
